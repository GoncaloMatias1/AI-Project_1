...
def main():
    ...
    min_arrival_time = get_input("Enter the minimum expected arrival time (in minutes): ", type_=float, min_=10, max_=120)
    max_arrival_time = get_input("Enter the maximum expected arrival time (in minutes): ", type_=float, min_=min_arrival_time, max_=120)
    ...
    for airplane_id, landing_time in optimized_schedule:
        ...
        if fuel_remaining < 0:
            ...
            print(f"Airplane {airplane_id} landed after {round(landing_time)} minutes with {round(fuel_remaining)} liters of fuel remaining.")
    ...
    save_simulation_results([(airplane_id, round(landing_time)) for airplane_id, landing_time in optimized_schedule], 'optimized_landing_schedule.txt')
