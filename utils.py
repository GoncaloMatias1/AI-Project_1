...
def log_airplane_data(airplane, first_airplane=False):
    ...
    with open(log_file_path, mode) as file:
        ...
        file.write(f"Expected Arrival Time: {airplane.expected_arrival_time} minutes\n")

        # Calculando o combustível utilizado
        fuel_used_calc = f"{airplane.fuel_consumption_rate} * ({airplane.expected_arrival_time}/60)"
        fuel_used = airplane.fuel_consumption_rate * (airplane.expected_arrival_time / 60)
